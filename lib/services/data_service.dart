...
class DataService {
  ...
  static Future<Post> storePost(Post post) async {
    ...
    await _firestore.collection(folder_users).document(me.uid).collection(
        folder_posts).document(postId).setData(post.toJson());
    return post;
  }
  ...
  static Future<Post> storeFeed(Post post) async {
    ...
    await _firestore.collection(folder_users).document(uid).collection(
        folder_feeds).document(post.id).setData(post.toJson());
    return post;
  }
  ...
  static Future<Post> likePost(Post post, bool liked) async {
    ...
    if (uid == post.uid) {
      await _firestore.collection(folder_users).document(uid).collection(
          folder_posts).document(post.id).setData(post.toJson());
    }
  }
  ...
  static Future<List<Post>> loadLikes() async {
    ...
    var querySnapshot = await _firestore.collection(folder_users).document(uid)
        .collection(folder_feeds).where("liked", isEqualTo: true)
        .getDocuments();
  ...
  static Future<User> followUser(User someone) async {
    ...
    await _firestore.collection(folder_users).document(me.uid).collection(
        folder_following).document(someone.uid).setData(someone.toJson());

    // I am in someone`s followers
    await _firestore.collection(folder_users).document(someone.uid).collection(
        folder_followers).document(me.uid).setData(me.toJson());
  ...
  static Future<User> unfollowUser(User someone) async {
    ...
    await _firestore.collection(folder_users).document(me.uid).collection(
        folder_following).document(someone.uid).delete();

    // I am not in someone`s followers
    await _firestore.collection(folder_users).document(someone.uid).collection(
        folder_followers).document(me.uid).delete();
  ...
  static Future storePostsToMyFeed(User someone) async {
    ...
    List<Post> posts = new List();
    var querySnapshot = await _firestore.collection(folder_users).document(
        someone.uid).collection(folder_posts).getDocuments();
    querySnapshot.documents.forEach((result) {
  ...
  static Future removeFeed(Post post) async {
    ...
    return await _firestore.collection(folder_users).document(uid).collection(
        folder_feeds).document(post.id).delete();
  }
  ...
  static Future<List<Post>> loadingPost(User someone)async{
    List<Post> postv=new List();
    var snapshots=await _firestore.collection(folder_users).document(someone.uid).collection(folder_posts).getDocuments();
  ...
  static Future <List<User>> loadUserOthers() async{
    String uid=await Prefs.loadUserId();
    List<User> items=List();
    var value=await _firestore.collection(folder_users).getDocuments();
